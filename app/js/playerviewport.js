...
PlayerViewport.prototype.render = function(renderer){
    ...
    if (this.game.gameState == 2 && this.followingEntity != null) {
        ...
        var position = new Vector2(this.width, this.height).add(new Vector2(this.x, this.y));
        ...
        renderer.drawText("Speed: " + Math.floor(this.followingEntity.getForwardSpeed()*7), position.clone().sub(new Vector2(20, 250)));
        ...
        renderer.draw(position.clone().subScalar(70));
...
PlayerViewport.prototype.followEntity = function(entity){
    ...
    if (!this.game.multiplayer && (this.game.isMousePressed(false) || this.game.isMousePressed(true)) && this.x <= position.x && this.x + this.width >= position.x && this.y <= position.y && this.y + this.height >= position.y){
